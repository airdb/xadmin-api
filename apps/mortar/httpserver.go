...
func HTTPServerBuilderFxOption() fx.Option {
	return fx.Provide(HTTPServerBuilder)
}
...
type httpServerDeps struct {
	...
	// Serverless
	Serverless *Serverless `optional:"true"`
	...
	ExternalHTTPHandlers         []partial.HTTPHandlerPatternPair         `group:"externalHttpHandlers"`
	ExternalHTTPHandlerFunctions []partial.HTTPHandlerFuncPatternPair     `group:"externalHttpHandlerFunctions"`
	ExternalHTTPInterceptors     []serverInt.GRPCGatewayInterceptor       `group:"externalHttpInterceptors"`

	// Internal REST
	InternalHTTPHandlers         []partial.HTTPHandlerPatternPair     `group:"internalHttpHandlers"`
	InternalHTTPHandlerFunctions []partial.HTTPHandlerFuncPatternPair `group:"internalHttpHandlerFunctions"`
...
func (deps httpServerDeps) buildExternalAPI(builder serverInt.GRPCWebServiceBuilder) serverInt.GRPCWebServiceBuilder {
	...
	if externalRESTPort.IsSet() && (len(deps.ExternalHTTPHandlerFunctions) > 0 || len(deps.ExternalHTTPHandlers) > 0 || len(deps.GRPCGatewayGeneratedHandlers) > 0) {
		...
		if deps.Serverless != nil {
			restBuilder = restBuilder.SetCustomServer(&http.Server{
				Handler: deps.Serverless.externalMux,
			})
		}
