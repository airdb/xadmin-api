// Refer: https://github.com/gertd/go-pluralize
package util
...
// NewPluralize - pluralization client factory method.
...
func (c *pluralize) Pluralize(word string, count int, inclusive bool) string {
...
func (c *pluralize) AddPluralRule(rule string, replacement string) {
	c.pluralRules = append(c.pluralRules, PluralizeRule{pluralizeSanitizeRule(rule), replacement})
}

// AddSingularRule -- Add a singularization rule to the collection.
func (c *pluralize) AddSingularRule(rule string, replacement string) {
	c.singularRules = append(c.singularRules, PluralizeRule{pluralizeSanitizeRule(rule), replacement})
}

// AddUncountableRule -- Add an uncountable word rule.
func (c *pluralize) AddUncountableRule(word string) {
	if !pluralizeIsExpr(word) {
...
func (c *pluralize) AddIrregularRule(single string, plural string) {
...
func (c *pluralize) replaceWord(replaceMap map[string]string, keepMap map[string]string, rules []PluralizeRule) func(w string) string { //nolint:lll
	f := func(word string) string {
		...
		if _, ok := keepMap[token]; ok {
			return pluralizeRestoreCase(word, token)
		}

		// Check against the replacement map for a direct word replacement.
		if replaceToken, ok := replaceMap[token]; ok {
			return pluralizeRestoreCase(word, replaceToken)
		}
...
func (c *pluralize) checkWord(replaceMap map[string]string, keepMap map[string]string, rules []PluralizeRule) func(w string) bool {
...
func (c *pluralize) interpolate(str string, args []string) string {
...
func (c *pluralize) replace(word string, rule PluralizeRule) string {
	return rule.expression.ReplaceAllStringFunc(word, func(w string) string {
		...
		if match == `` {
			return pluralizeRestoreCase(word[index-1:index], result)
		}
		return pluralizeRestoreCase(match, result)
	})
}

func (c *pluralize) sanitizeWord(token string, word string, rules []PluralizeRule) string {
	// If empty string
	if len(token) == 0 {
		return word
	}
...
func pluralizeSanitizeRule(rule string) *regexp.Regexp {
	if pluralizeIsExpr(rule) {
...
func pluralizeRestoreCase(word string, token string) string {
...
// pluralizeIsExpr -- helper to detect if string represents an expression by checking first character to be `(`.
func pluralizeIsExpr(s string) bool {
	return s[:1] == `(`
}

func (c *pluralize) loadIrregularRules() { //nolint:funlen
...
func (c *pluralize) loadPluralizationRules() {
...
func (c *pluralize) loadSingularizationRules() {
...
func (c *pluralize) loadUncountableRules() { //nolint:funlen
